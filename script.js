window.addEventListener('load', () => {
  ...
  const grassMargin = 50; // space above button

  // ----- Grass -----
  function getGrassTop() {
    const buttonRect = addButton.getBoundingClientRect();
    const canvasRect = canvas.getBoundingClientRect();
    return (buttonRect.top - canvasRect.top) - grassMargin; // top of grass above button
  }
  ...
  class Sunflower {
    constructor(x, grassTop) {
      this.x = x;
      this.stemTop = grassTop; // stem starts at grass top
      this.stemHeight = 120;
      this.stemWidth = 6;
      this.maxRadius = 30;
  ...
  class Butterfly {
    ...
    draw(ctx) {
      ...
      if (this.y < -20) return false; // remove if out of top
      return true;
    }
  }
  ...
  function animate() {
    ...
    floatingTexts.forEach((t, i) => {
      t.draw(ctx);
      if (t.life > t.totalLife) floatingTexts.splice(i, 1);
    });
